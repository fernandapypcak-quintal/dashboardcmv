...
export async function loadCMVData() {
  const [resFichas, resDesperdicio, resHistorico, resHistProd, resHistComp, resVendas, resParams, resHistory] = await Promise.all([
    fetchTipo('fichas'),
  ...
  console.log(`[CMV] fichas=${fichas.length} histórico=${historico.length} desperdício=${desperdicio.length} histProd=${histProd.length} histComp=${histComp.length} vendas=${vendas.length} history=${history.length}`);
