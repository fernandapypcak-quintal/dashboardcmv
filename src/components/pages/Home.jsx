...
export default function Home({ onPageChange }) {
  const {
    kpis, evolucao, desperdicioByUnidade,
    produtos, margemPorCategoria, histComp, PorContabil,
    setFiltroCat, setFiltroLoja,
  } = use();

  const [painelProduto, setPainelProduto] = useState(null);

  const criticos = produtos.filter(r => r.Pct >= 0.80);
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div onClick={() => onPageChange('variacao')}
          ...
          <p className="text-[10.5px] font-semibold text-zinc-400 uppercase tracking-wide mb-2"> Médio Teórico</p>
          <p className={`text-[28px] font-bold leading-none tracking-tight ${kpis.Atual > 0.30 ? 'text-amber-700' : 'text-brand-olive'}`}>
            {pct(kpis.Atual)}
          </p>
          <p className={`text-[12px] font-medium mt-1.5 ${kpis.delta > 0 ? 'text-brand-crimson' : 'text-brand-olive'}`}>
            {kpis.delta > 0 ? '↑' : '↓'} {pct(Math.abs(kpis.delta))} vs sem. ant.
          </p>
        </div>
      ...
      {criticos.length > 0 && (
        <div className="bg-white border border-surface-border rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-surface-border flex items-center justify-between">
            <div>
              <p className="font-semibold text-brand-black text-sm"> acima de 80% — prejuízo a cada venda</p>
              ...
              return (
                <div key={item.codPa}
                  ...
                  <div className="shrink-0 w-16 text-center">
                    <p className="text-[18px] font-bold text-brand-crimson leading-none">{pct(item.Pct)}</p>
                    <p className="text-[9px] text-zinc-400 mt-0.5 uppercase tracking-wide"></p>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white border border-surface-border rounded-xl">
          <div className="px-5 py-3.5 border-b border-surface-border flex items-center justify-between">
            <div>
              <p className="font-semibold text-brand-black text-sm">Evolução do </p>
            ...
            {evolucao.length > 0 ? (
              <ResponsiveContainer width="100%" height={190}>
                <LineChart data={evolucao} margin={{top:4,right:8,left:-20,bottom:0}}>
                  ...
                  <Line type="monotone" dataKey="" name="Geral" stroke="#0D0D0D" strokeWidth={2.5} dot={{fill:'#0D0D0D',r:3}} activeDot={{r:5}}/>
                  {cats.map((cat,i) => (
          ...
          {margemPorCategoria.map(r => (
            <div key={r.categoria}
              ...
              <div className="flex items-end justify-between">
                <div>
                  <p className={`text-[20px] font-bold leading-none ${r.Medio>=0.80?'text-brand-crimson':r.Medio>=0.35?'text-amber-700':'text-brand-olive'}`}>
                    {pct(r.Medio)}
                  </p>
                  <p className="text-[10px] text-zinc-400 mt-1"> médio</p>
                </div>
